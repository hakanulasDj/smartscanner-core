...
private constructor() {
  ...
  init {
    ...
    private fun doActiveAuth() {
        // Active Authentication using Data Group 15
        var dg15Encoded = ByteArray(0)
        try {
            val dg15In: CardFileInputStream = (service as PassportService).getInputStream(PassportService.EF_DG15)
            dg15Encoded = IOUtils.toByteArray(dg15In)
            val dg15InByte = ByteArrayInputStream(dg15Encoded)
            var m_dg15File = DG15File(dg15InByte)
            val publicKey = m_dg15File.publicKey
            val random = SecureRandom()
            val challenge = ByteArray(8)
            random.nextBytes(challenge)
            (service as PassportService).doAA(publicKey, null, "SHA256withRSA", challenge)
